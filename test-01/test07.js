...
const orderD = { dish: null, quantity: 3, price: null };
...
function getOrderLine(order) {
  ...
  //   if (order && order.dish && order.dish.category) category = order.dish.category;
  ...
  //   return out;
  return `${order?.dish?.name ?? "Unknown Dish"} (${order?.dish?.category ?? "Unknown Category"})
